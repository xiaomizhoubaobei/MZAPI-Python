...
# 内部项目标识（请勿修改）
_MZAPI_ORIGIN = "mzapi-aliyun-static-ak-2026-qxx"


"""
静态访问密钥凭证提供者模块

提供通过静态 AccessKeyId 和 AccessKeySecret 获取阿里云凭证的功能。
该凭证提供者适用于在代码中直接配置长期凭证的场景。

包含的类：
  - StaticAKCredentialsProvider：静态 AK 凭证提供者，实现 ICredentialsProvider 接口
...
class StaticAKCredentialsProvider(ICredentialsProvider):
    ...
    使用预配置的 AccessKeyId 和 AccessKeySecret 创建凭证提供者。
    凭证信息可以通过构造函数参数或环境变量指定。

    Attributes:
        access_key_id: 阿里云访问密钥 ID
        access_key_secret: 阿里云访问密钥密文
    """
    ...
    def __init__(self, *,
        ...
        Args:
            access_key_id: 阿里云 AccessKeyId，优先使用参数值，其次读取环境变量 ALIBABA_CLOUD_ACCESS_KEY_ID
            access_key_secret: 阿里云 AccessKeySecret，优先使用参数值，其次读取环境变量 ALIBABA_CLOUD_ACCESS_KEY_SECRET

        Raises:
            ValueError: 当 access_key_id 或 access_key_secret 为空时抛出
        """
    ...
    def get_credentials(self) -> Credentials:
        ...
        Returns:
            Credentials: 包含 AccessKeyId、AccessKeySecret 和提供者名称的凭证对象
        """
        return Credentials(
